...
class _LastsendviewState extends State<Lastsendview> {
  ...
  Future<void> fetchHistoryData() async {
    ...
    try {
      final response = await http.get(
          Uri.parse(Apiendpoints.baseUrl + Apiendpoints.production.history)).timeout(Duration(minutes: 20));
  ...
  "locationProcess": "اخر عملية انتاج",
          'items': items
        }),
...
void showEditDialog(Map<String, dynamic> item) {
  ...
  for (var productItem in items) {
    controllers.add(TextEditingController(text: productItem['qty'].toString()));
  }
                  ...
                  child: SingleChildScrollView(
                    child: ConstrainedBox(
                      constraints: BoxConstraints(
                        maxHeight: MediaQuery.of(context).size.height * (isWide ? 0.7 : 0.6),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            height: MediaQuery.of(context).size.height * (isWide ? 0.45 : 0.35),
